...
implementation
    ...
    begin
        ...
        begin
            found := false;
            start := list.last^.sig; // Guarda el primer nodo
            prev := list.last; // Nodo previo al nodo actual
            aux := start;
            repeat
                if aux^.info = x then
                begin
            ...
            until found or (aux = start);
            if (list.last^.sig = list.last) and (list.last^.info = x) then
            begin
                dispose(list.last);
                list.last := nil;
            end;
